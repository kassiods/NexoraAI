      ...
      {items.map((item) => {
        ...
        return (
          <Link
            ...
            className={`group relative flex items-center gap-3 rounded-xl px-3 py-2 transition duration-150 ease-out focus-visible:ring-2 focus-visible:ring-[color:rgba(231,233,234,0.14)] focus-visible:ring-offset-0 ${
              active
                ? 'bg-[var(--bg-surface)] text-[var(--action-hover)]'
                : 'text-[var(--text-secondary)] hover:bg-[var(--bg-surface-hover)] hover:text-[var(--text-primary)]'
            }`}
          >
            <span className={`absolute left-0 top-1/2 h-8 w-1.5 -translate-y-1/2 rounded-full transition duration-150 ease-out ${active ? 'bg-[var(--action)]' : 'bg-transparent'}`} />
      ...
      {items.map((item) => {
        ...
        return (
          <Link
            ...
            href={item.href}
            aria-current={active ? 'page' : undefined}
            className={`flex flex-col items-center gap-1 transition duration-150 ease-out ${active ? 'text-[var(--action-hover)] font-semibold' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
          >
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const navItems: NavItem[] = useMemo(() => {
    const base: NavItem[] = [
      { href: '/dashboard', label: 'Inicio', icon: <Home className="h-4 w-4 text-white" /> },
      ...
      <div className="flex min-h-screen">
        ...
        >
          <div className="flex items-center justify-between px-2">
            <Link href="/dashboard" className="flex items-center gap-2 text-lg font-semibold text-[var(--text-primary)] transition duration-150 ease-out hover:text-[var(--action-hover)]">
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--action)] text-black">
                N
              </span>
              <span>Nexora</span>
            </Link>
            <button className="lg:hidden text-[var(--text-secondary)] transition duration-150 ease-out hover:text-[var(--action-hover)]" onClick={() => setMobileOpen(false)} aria-label="Fechar menu">
              <X className="h-5 w-5" />
            </button>
          </div>
        ...
        <div className="flex min-h-screen flex-1 flex-col lg:pl-0">
          <header className="sticky top-0 z-10 flex items-center justify-between border-b border-[var(--border)] bg-[color:rgba(10,10,10,0.9)] px-4 py-3 backdrop-blur lg:hidden">
            <div className="flex items-center gap-3">
              <button className="rounded-lg border border-[var(--border)] bg-[var(--bg-surface)] p-2 text-[var(--text-primary)] transition duration-150 ease-out hover:bg-[var(--bg-surface-hover)]" onClick={() => setMobileOpen(true)} aria-label="Abrir menu">
                <Menu className="h-5 w-5" />
              </button>
              <Link href="/dashboard" className="text-base font-semibold text-[var(--text-primary)] transition duration-150 ease-out hover:text-[var(--action-hover)]">
                Nexora
              </Link>
            </div>
